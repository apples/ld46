...
function version_selector(props)
    ...
    local highscore = math.floor(get_highscore())
    ...
    return vdom.create_element('widget', { width='50%', height='100%', halign='right', right=0 },
        ...
        vdom.create_element('label', { color='#fff', height=20, left=-98, bottom=280, text='Highscore: '..highscore })
    )
end
...
function update_gui_state(props)
    vdom_root = vdom.render(vdom.create_element(version_selector, props), root_widget, vdom_root)
    vdom.flush_updates()
end
